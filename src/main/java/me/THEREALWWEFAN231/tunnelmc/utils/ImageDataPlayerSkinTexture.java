...
@Log4j2
public class ImageDataPlayerSkinTexture extends ResourceTexture {
    ...
    public void load(ResourceManager manager) throws IOException {
        TunnelMC.mc.executeSync(() -> {
            if (!this.loaded) {
                try {
                    ...
                    log.warn("Failed to load texture: {}", this.location, var3);
                }
        ...
        log.debug("Loading skin texture from imageData");
        NativeImage nativeImage = this.loadTexture(this.image);
        if(nativeImage == null) {
    ...
    private NativeImage loadTexture(ImageData image) {
        ...
        try {
            nativeImage = SkinUtils.toNativeImage(image);
            if (this.convertLegacy) {
                nativeImage = remapTexture(nativeImage);
            }
        } catch (Exception e) {
            log.error("Error while loading the skin texture", e);
        }
    ...
    private static NativeImage remapTexture(NativeImage image) {
        ...
        if (j == 64 && (i == 32 || i == 64)) {
            ...
            image.close();
            log.warn("Discarding incorrectly sized ({}x{}) skin texture", new Object[]{j, i});
            return null;
        }
    }
