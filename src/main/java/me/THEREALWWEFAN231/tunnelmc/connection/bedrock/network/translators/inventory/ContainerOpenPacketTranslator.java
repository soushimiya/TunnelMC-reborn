...
public class ContainerOpenPacketTranslator extends PacketTranslator<ContainerOpenPacket> {
	...
	@Override
	public void translate(ContainerOpenPacket packet, Client client) {
		if (packet.getType() == ContainerType.INVENTORY) {
			return;
		}
		
		client.openContainerId = packet.getId();
		...
		NbtMap blockEntityData = client.blockEntityDataCache.getDataFromBlockPosition(packet.getBlockPosition());
		...
		client.javaConnection.processServerToClientPacket(openScreenS2CPacket);

		client.containers.setCurrentlyOpenContainer(new BedrockContainer(27, packet.getId()));
	}
}
